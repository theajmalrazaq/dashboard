...
export const POST: APIRoute = async ({ request, url }) => {
  ...
  if (!command) {
    return new Response(
      JSON.stringify({ success: false, error: "No command" }),
      {
        status: 400,
        headers: { "Content-Type": "application/json" },
      },
  ...
  try {
    if (command === "playpause") {
      ...
      if (!status) {
        asyncRun(
          'sh -c "spotify & sleep 1.5 && hyprctl dispatch movetoworkspacesilent special,class:spotify && sleep 1 && playerctl -p spotify play-pause"',
        );
      } else {
    ...
    // Trigger instant broadcast update over WebSocket if running
