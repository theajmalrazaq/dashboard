...
export default function Terminal({ isActive }) {
  ...
  const executeCommand = async (cmd) => {
    setHistory((prev) => [
      ...prev,
      { type: "command", content: cmd, path: cwd.replace(DEFAULT_HOME, "~") },
    ]);
    ...
    if (baseCmd === "help") {
      setHistory((prev) => [
        ...
        {
          type: "system",
          content: "Available: clear, help, cd, ls, and system commands.",
        },
      ]);
      return;
    }
    ...
    if (baseCmd === "cd") {
      ...
      try {
        const res = await fetch(
          `/api/system?action=validate&path=${encodeURIComponent(newPath)}`,
        );
        ...
        setHistory((prev) => [
          ...prev,
          { type: "stderr", content: "cd: validation failed." },
        ]);
      }
      return;
    }
    ...
    try {
      ...
      if (data.stdout) {
        setHistory((prev) => [
          ...prev,
          { type: "stdout", content: data.stdout },
        ]);
      }
      if (data.stderr) {
        setHistory((prev) => [
          ...prev,
          { type: "stderr", content: data.stderr },
        ]);
      }
      ...
      setHistory((prev) => [
        ...prev,
        { type: "stderr", content: "Network error." },
      ]);
    } finally {
      setExecuting(false);
    }
        ...
        {history.map((line, i) => (
          <div key={i} className="break-words leading-relaxed">
            {line.type === "command" ? (
              ...
              <div className="text-blue-400 opacity-80 italic italic">
                {line.content}
              </div>
            ) : line.type === "stderr" ? (
              <div className="text-red-400/90 bg-red-500/5 px-2 py-1 rounded-lg border border-red-500/10 whitespace-pre-wrap">
                {line.content}
              </div>
            ) : (
