  ...
  const updateNestedItem = (items, targetId, updater) => {
    return items.map((item) => {
      ...
      if (item.items?.length > 0) {
        return {
          ...item,
          items: updateNestedItem(item.items, targetId, updater),
        };
      }
      return item;
    });
  ...
  const toggleTodo = useCallback(
    async (rootTodo, itemId, isRoot) => {
      ...
      const toggleWithChildren = (item, status) => {
        ...
        if (updated.items) {
          updated.items = updated.items.map((child) =>
            toggleWithChildren(child, status),
          );
        }
        return updated;
      };
        ...
        onConfirm: async () => {
          if (isRoot) {
            const { error } = await supabase
              ...
              .eq("id", rootTodo.id);
            if (!error)
              setTodos((prev) => prev.filter((t) => t.id !== rootTodo.id));
          } else {
            ...
            if (!error)
              setTodos((prev) =>
                prev.map((t) => (t.id === rootTodo.id ? data : t)),
              );
          }
          ...
          {loading && (
            <div className="flex flex-col gap-4 max-w-2xl mx-auto w-full">
              ...
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 p-4 border-b border-gray-100 dark:border-neutral-900"
                >
...
const TodoItem = memo(
  ({ todo, rootTodo, depth = 0, onToggle, onAddSub, onDelete, togglingId }) => {
        ...
        >
          <div className="flex items-center gap-2">
            ...
            >
              {togglingId === todo.id ? (
                <span
                  className={`w-2 h-2 border-2 ${todo.completed ? "border-white" : "border-accent"} border-t-transparent rounded-full animate-spin`}
                ></span>
              ) : (
                todo.completed && (
                  <i className="hgi-stroke hgi-tick-01 text-[10px] font-bold"></i>
                )
              )}
            </button>
          </div>
