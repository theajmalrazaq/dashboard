...
export default function FileExplorer({ isActive }) {
  ...
  const readDirectory = async (targetPath) => {
    ...
    try {
      const res = await fetch(
        `/api/system?action=ls&path=${encodeURIComponent(targetPath)}`,
      );
  ...
  const handleItemClick = async (item) => {
    if (item.kind === "directory") {
      ...
      if (
        ["txt", "md", "js", "json", "html", "css", "ts", "py"].includes(
          extension,
        )
      )
        type = "text";
      ...
      if (type === "text") {
        try {
          const res = await fetch(
            `/api/system?action=read&path=${encodeURIComponent(item.path)}`,
          );
          const data = await res.json();
      ...
      <div className="flex justify-between items-center mb-4 px-2">
        <div className="flex items-center gap-3">
          {path !== DEFAULT_HOME && (
            <button
              onClick={goBack}
              className="cursor-pointer text-gray-600 hover:text-accent transition-colors"
            >
              <i className="hgi-stroke hgi-arrow-left-01 text-lg"></i>
            </button>
          )}
          <div className="flex flex-col">
            <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100 font-product-sans lowercase truncate max-w-50 sm:max-w-md">
              {path.replace(DEFAULT_HOME, "~") || "root"}
            </h3>
            <p className="text-[10px] text-gray-600 dark:text-neutral-600 font-product-sans lowercase">
              {items.length} items
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-50 dark:bg-neutral-900 text-gray-600 hover:text-accent transition-all cursor-pointer border border-gray-100 dark:border-neutral-800"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4">
        {items.map((item, idx) => (
          <button
            ...
            className="group flex flex-col items-center gap-3 p-4 bg-transparent hover:bg-gray-400/5 dark:hover:bg-white/2 rounded-4xl border border-transparent hover:border-gray-100 dark:hover:border-neutral-900 transition-all duration-500 cursor-pointer"
          >
            <div
              className={`w-16 h-16 flex items-center justify-center rounded-3xl bg-gray-50 dark:bg-neutral-900/50 text-gray-600 group-hover:text-accent group-hover:bg-accent/5 transition-all duration-300 transform group-hover:scale-110 border border-transparent group-hover:border-accent/10 relative overflow-hidden`}
            >
        ...
        {items.length === 0 && !loading && (
          <div className="col-span-full py-20 bg-gray-50/50 dark:bg-neutral-950/20 border-2 border-dashed border-gray-100 dark:border-neutral-900 rounded-4xl flex flex-col items-center justify-center text-gray-600">
        ...
        <div className="h-[85vh] flex flex-col relative bg-white dark:bg-black">
          ...
          <div className="absolute top-6 left-6 right-6 flex items-center justify-between z-10">
            <div className="bg-black/50 backdrop-blur-xl px-6 py-2 rounded-full border border-white/10">
              <h3 className="text-sm font-bold text-white font-product-sans">
                {preview?.name}
              </h3>
            </div>
            <button
          ...
          <div className="flex-1 flex items-center justify-center overflow-auto p-12">
            {preview?.type === "text" ? (
              ...
              <div className="text-center">
                <i
                  className={`hgi hgi-stroke ${getIcon({ name: preview?.name, kind: "file" })} text-6xl text-gray-600 mb-4 opacity-50`}
                ></i>
                <p className="text-lg font-product-sans text-gray-700">
                  Preview not available via System API yet.
                </p>
