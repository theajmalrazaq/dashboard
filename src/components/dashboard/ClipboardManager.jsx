...
export default function ClipboardManager({ clipboardData }) {
  ...
  // Synchronize local clipboard state with global SSE stream updates
  useEffect(() => {
    if (clipboardData) {
      if (clipboardData.history) setHistory(clipboardData.history);
      if (clipboardData.current !== undefined)
        setCurrent(clipboardData.current);
      setLoading(false);
    }
  }, [clipboardData]);
  ...
  const cleanText = (text) => {
    ...
    if (
      text.length > 5000 ||
      /[\x00-\x08\x0E-\x1F\x7F]/.test(text.substring(0, 100))
    ) {
          ...
          {filteredHistory.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col">
                  <p className="font-bold text-gray-900 dark:text-gray-100 font-product-sans text-sm leading-relaxed truncate">
                    {item.type === "file"
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  ...
                  >
                    <i
                      className={`hgi hgi-stroke ${copying === item.id ? "hgi-check-mark-circle-01" : "hgi-copy-01"} text-lg`}
                    ></i>
                  </button>
                </div>
              </div>
