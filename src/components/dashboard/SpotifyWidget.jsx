...
export default function SpotifyWidget({ spotifyState }) {
  const [track, setTrack] = useState(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [loading, setLoading] = useState(true);
  const [progress, setProgress] = useState({ position: 0, duration: 0 });

  // Synchronize state with SSE/WS stream updates
  useEffect(() => {
    if (spotifyState) {
      if (spotifyState.active) {
        setTrack(spotifyState.track);
  ...
  // Interpolate progress smoothly locally every second when playing
  useEffect(() => {
    if (!isPlaying) return;
      ...
      <div className="relative flex flex-col bg-gray-50/50 dark:bg-neutral-900/20 border border-gray-100 dark:border-neutral-800/50 rounded-3xl transition-all duration-500 hover:border-accent/10 overflow-hidden">
        <div className="flex items-center gap-6 p-4">
          ...
          <div className="flex items-center gap-2 border-l border-gray-100 dark:border-neutral-800/50 pl-6">
            ...
            >
              <i
                className={`hgi-stroke ${isPlaying ? "hgi-pause" : "hgi-play"} text-base`}
              ></i>
            </button>
            <button
